		...
		if (!$error) {
			...
			if (!$error) {
				...
				foreach($stores as $store){
					...
					$query = "INSERT INTO `llx_ticket` (`entity`, `ref`, `track_id`, `fk_soc`, `fk_project`, `fk_user_create`, `subject`, `message`, `fk_statut`, `type_code`, `category_code`, `severity_code`, `datec`) VALUES ('1', '" . $defaultref . "', '". $trackid ."', '" . $object->array_options["options_customerbranch"] . "', '".$object->id."', '".$user->id."', '".$ticketsubject."', '".$object->description."', '0', 'Roll', 'INSTA OT', 'Termin', '".date("Y-m-d H:i:s")."')";
					$db->query($query, 'ddl');
					$iid = $db->last_insert_id('llx_ticket');
					$query_extra = "INSERT INTO `llx_ticket_extrafields` (`fk_object`, `fk_store`, `customer`, `ordervia`) VALUES ('" . $iid . "', '". $store ."', '" . $object->socid . "', 'Storema')";
...
if ($action == 'create' && $user->hasRight('projet', 'creer')) {
	...
	print '<script>
		...
		$sql = "SELECT DISTINCT LEFT(s.zip_code, 1) AS `zipcode`, 
							...
							LEFT JOIN llx_projet_extrafields p ON p.stores LIKE CONCAT('%', ss.rowid, '%') 
							...
							WHERE t.fk_statut = 8 AND t.fk_soc = s.fk_soc AND LEFT(sb.zip_code, 1) = LEFT(s.zip_code, 1) AND pr.rowid = ".$id." AND s.country_id = ".$countryIDFilter.") AS `total_stores_done`,
							(SELECT COUNT(DISTINCT te.fk_store) 
							FROM llx_ticket_extrafields te 
							JOIN llx_ticket t ON te.fk_object = t.rowid 
							JOIN llx_stores_branch sb ON te.fk_store = sb.rowid
							JOIN llx_projet pr ON t.fk_project = pr.rowid
							WHERE t.fk_statut <> 8 AND t.fk_statut <> 0 AND t.fk_soc = s.fk_soc AND LEFT(sb.zip_code, 1) = LEFT(s.zip_code, 1) AND pr.rowid = ".$id." AND s.country_id = ".$countryIDFilter.") AS `total_stores_open`
