...
/**
 *   	\file       poshardware_list.php
 *		\ingroup    stores
 *		\brief      List page for poshardware
 */
...
require_once __DIR__.'/class/poshardware.class.php';
...
$hookmanager->initHooks(array('poshardwarelist')); // Note that conf->hooks_modules contains array
...
// 	$permissiontoadd = $user->rights->stores->poshardware->write;
// 	$permissiontodelete = $user->rights->stores->poshardware->delete;
// } else {
// 	$permissiontoread = 1;
// 	$permissiontoadd = 1;
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'PosHardware';
	$objectlabel = 'PosHardware';
...
//$help_url = "EN:Module_PosHardware|FR:Module_PosHardware_FR|ES:Módulo_PosHardware";
$help_url = '';
if($id){
	$object1->fetch($id);
	$title = $object1->b_number." - ".$langs->trans("POS Hardwares");
	...
	$title = $langs->trans("PosHardwares");
}
$morejs = array();
$morecss = array();
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/stores/poshardware_card.php', 1).'?id='.$id);
	exit;
}
...
if($id){
	...
	print dol_get_fiche_head($head, 'pos', $langs->trans("Branch"), -1, $object1->picto);
...
if($id){
	$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/stores/poshardware_card.php', 1).'?store='.$id.'&action=create&backtopage='.urlencode($_SERVER['PHP_SELF']).'?id='.$id, '', $permissiontoadd);	
}else{
	$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/stores/poshardware_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);
}
print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendPosHardwareRef";
$modelmail = "poshardware";
$objecttmp = new PosHardware($db);
...
// $db->free($resql);
//-----------------------------NEW CODE--------------------------------------------
print '<div class="box" id="boxto_3">
			<table summary="boxtable3" width="30%">
				<tbody>
					<tr class="nohover">
						<td>
							<a href="" class="boxstatsindicator thumbstat nobold nounderline">
								<div class="boxstats">
									<span class="left" style="font-size:15px">
										Kasse
									</span>
									...
									<span style="font-size:20px">
										<img src="https://cdn-icons-png.flaticon.com/512/4305/4305365.png" width="25">
									</span>
									<span style="font-size:15px">
										31
									</span>
