...
/**
 *   	\file       mediahardware_list.php
 *		\ingroup    stores
 *		\brief      List page for mediahardware
 */
...
require_once __DIR__.'/class/mediahardware.class.php';
...
$hookmanager->initHooks(array('mediahardwarelist')); // Note that conf->hooks_modules contains array
...
// 	$permissiontoadd = $user->rights->stores->mediahardware->write;
// 	$permissiontodelete = $user->rights->stores->mediahardware->delete;
// } else {
// 	$permissiontoread = 1;
// 	$permissiontoadd = 1;
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'MediaHardware';
	$objectlabel = 'MediaHardware';
...
//$help_url = "EN:Module_MediaHardware|FR:Module_MediaHardware_FR|ES:Módulo_MediaHardware";
$help_url = '';
if($id){
	$object1->fetch($id);
	$title = $object1->b_number." - ".$langs->trans("Media Hardwares");
	...
	$title = $langs->trans("MediaHardwares");
}
$morejs = array();
$morecss = array();
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/stores/mediahardware_card.php', 1).'?id='.$id);
	exit;
}
...
if($id){
	...
	print dol_get_fiche_head($head, 'media', $langs->trans("Branch"), -1, $object1->picto);
...
if($id){
	$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/stores/mediahardware_card.php', 1).'?store='.$id.'&action=create&backtopage='.urlencode($_SERVER['PHP_SELF']).'?id='.$id, '', $permissiontoadd);
}else{
	$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/stores/mediahardware_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);
}

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendMediaHardwareRef";
$modelmail = "mediahardware";
$objecttmp = new MediaHardware($db);
