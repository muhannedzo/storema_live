...
if ($action == 'create' || $action == 'presend') {
	...
	if ($res > 0) {
		...
		//Muhannad's Code
		if(str_contains($_SERVER['HTTP_REFERER'], "store_history")){
			$linkback = '<a href="'.$_SERVER['HTTP_REFERER'].'"><strong>'.$langs->trans("BackToList").'</strong></a> ';
		}else{
			$linkback = '<a href="'.DOL_URL_ROOT.'/ticket/list.php?restore_lastsearch_values=1"><strong>'.$langs->trans("BackToList").'</strong></a> ';
		}
		//End Muhannad's code
		...
		*/
		...
		// Date of use
		print '<tr><td>';
		print $langs->trans("Date of use / Estimated duration");
		print '</td><td>';
		$customer = $object->array_options["options_customer"];
		$ordervia = $object->array_options["options_ordervia"];
		$store = $object->array_options["options_fk_store"];
		$externalNumber = $object->array_options["options_externalticketnumber"];
		if(GETPOST('action', 'alpha') == "edit_extras" && GETPOST('attribute', 'alpha') == "options_dateofuse"){
			print '<form enctype="multipart/form-data" action="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'" method="post" name="formextra">
						<input type="hidden" name="action" value="update_extras">
						<input type="hidden" name="options_customer" value="'.$customer.'">
						<input type="hidden" name="options_ordervia" value="'.$ordervia.'">
						<input type="hidden" name="options_fk_store" value="'.$store.'">
						...
						print '<input type="submit" class="button" value="Modify">
				   </form>';
		}else{
		...
		$( document ).ready(function() {
			...
			for (var j = 0; j < tables.length; j++) {
			  ...
			  for (var i = 0; i < rows.length; i++) {
				...
				var cells2 = rows[i].getElementsByClassName("ticket_extras_timehour");
				if (cells2.length > 0) {
				  rows[i].style.display = "none";
				}
				var cells3 = rows[i].getElementsByClassName("ticket_extras_timeminute");
				if (cells3.length > 0) {
				  rows[i].style.display = "none";
				}
